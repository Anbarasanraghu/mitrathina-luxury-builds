...
interface DomeItem extends ImageData {
  x: number;
  y: number;
  sizeX: number;
  sizeY: number;
...
/**
 * Build grid items for the 3D dome
 * Creates a cylindrical arrangement of image tiles
 */
function buildItems(pool: ImageData[], seg: number): DomeItem[] {
  // Create column positions from -37 to +37 (adjustable range)
  const xCols = Array.from({ length: seg }, (_, i) => -37 + i * 2);
  const evenYs = [-4, -2, 0, 2, 4];  // Y positions for even columns
  const oddYs = [-3, -1, 1, 3, 5];   // Y positions for odd columns (offset)

  // Create coordinate grid with staggered rows
  ...
  // Repeat images to fill all slots
...
/**
 * Calculate 3D rotation for an item based on its position
...
function computeItemBaseRotation(
  ...
  const unit = 360 / segments / 2;  // Angular unit per segment
  const rotateY = unit * (offsetX + (sizeX - 1) / 2);  // Horizontal rotation
  const rotateX = unit * (offsetY - (sizeY - 1) / 2);  // Vertical rotation
...
export default function DomeGallery({
  ...
  // State refs for rotation and interaction
  ...
  // Start inertia scrolling after drag
  ...
  useGesture(
    {
      onDragStart: ({ event }) => {
        ...
        const potential = (e.target as HTMLElement).closest?.('.item__image') as HTMLElement;
  ...
  // Handle image enlargement close
  useEffect(() => {
    ...
    <>
      ...
      >
        ...
        >
          ...
          {/* Viewer frame and scrim for enlarged images */}
          <div
            ref={viewerRef}
